...
const StatusPage = () => {
  ...
  useEffect(() => {
    const fetchInvoices = async () => {
      try {
        ...
        const res = await fetch("https://tssezl-invoice-system.onrender.com/api/invoices/all", {
  ...
  const downloadExcel = async () => {
    try {
      const token = localStorage.getItem("token");
      const res = await fetch("https://tssezl-invoice-system.onrender.com/api/invoices/download/excel", {
        headers: { Authorization: `Bearer ${token}` },
      });
                ...
                <tr key={inv._id}>
                  ...
                  <td>
                    {inv.pdfs.map((file, i) => (
                      <div key={i}>
                        <a
                          href={`https://tssezl-invoice-system.onrender.com/uploads/${file}`}
                          target="_blank"
                          rel="noreferrer"
                        >
                          PDF {i + 1}
